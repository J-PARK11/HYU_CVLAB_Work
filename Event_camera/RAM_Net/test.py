...
def main(config, initial_checkpoint, output_folder, data_folder):
    ...
    os.environ['PREPROCESSED_DATASETS_FOLDER'] = 'home/work/main/jpark/Event_camera/data/Town05_test'
    ...
    if output_folder:
        ...
        preview = join(output_folder, "preview")
    ...
    if data_folder is not None:
        ...
        base_folder['validation'] = join(preprocessed_datasets_folder, '/home/work/main/jpark/Event_camera/data/Town05_test/')
    ...
    with torch.no_grad():
            ...
            if idx > 20 and idx < 24:
                ...
                for i, key in enumerate(input.keys()):
                    if "depth" in key:
                        ...
                        ax[3, index_2].set_title("input image" + key[-1])
                        index_2 += 1
                
                plt.savefig(join(preview, 'preview_{:04d}.png'.format(idx)), bbox_inches='tight')
                plt.show()
            ...
            if output_folder and sequence_idx > 1:
                ...
                for key in keys:
                    ...
                    if "event" in key:
                        ...
                        if video_idx > 20 and video_idx < 24:
                            fig, ax = plt.subplots(ncols=1, nrows=4, figsize=(10,20))
                            ax[0].imshow(negativ_input)
                            ax[0].set_title("neg input")
                            ax[1].imshow(positiv_input)
                            ax[1].set_title("pos input")
                            ax[2].imshow(zeros_input)
                            ax[2].set_title("zero input")
                            ax[3].imshow(total_image)
                            ax[3].set_title("total input")
                            plt.savefig(join(preview, 'event_preview_{:04d}.png'.format(video_idx)), bbox_inches='tight')
                            plt.show()
        ...
        print("\ntotal metrics:")
        print(metrics)
        print((np.sum(np.array(total_metrics), 0) / len(total_metrics)).round(4))
        print('\n>>>>>>>>>>>>> Complete <<<<<<<<<<<<<')
