...
def create_parser():
    ...
    parser.add_argument('--out_root', default='./output/temp/', type=str)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (input_img, gt_img, input_path, gt_path) in enumerate(tqdm(self.test_loader)):
                ...
                self.ft0 = softsplat.FunctionSoftsplat(tenInput=self.f10*0.5, tenFlow=self.f10*0.5,     
                                                        tenMetric=self.d1, strType='softmax')     
                self.ft1 = softsplat.FunctionSoftsplat(tenInput=self.f01*0.5, tenFlow=self.f01*0.5,     # [4,2,512,960]
