...
class raft:
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (input_img, gt_img, input_path, gt_path) in enumerate(tqdm(self.test_loader)):
                ...
                # self.fill_ft0, self.fill_ft1 = self.fill_flow()
                # print(self.fill_ft0.shape)
                ...
                # Synthesis
                self.syn_i_gt = self.depth_guide_synthesis()
                print(f'syn_i_gt: {self.syn_i_gt.shape}')

                # self.syn_i_gt = (self.git0gt + self.git1gt) / 2             # [4,3,512,960]
                self.syn_i = (self.git0 + self.git1) / 2
    ...
    def depth_guide_synthesis(self):
        for b in range(len(self.git0gt)):
            mask = torch.zeros_like(self.git0gt[b])
            condition = (self.d0t[b] > self.d1t[b]).repeat(3,1,1)
            mask[condition] = 1
            valid_i0 = mask * self.git0gt[b]
            valid_i1 = (1-mask) * self.git1gt[b]
            syn_i = valid_i0 + valid_i1
            if b == 0:
                result = syn_i
            else:
                result = torch.cat((result,syn_i))
        result = result.view((-1, 3, 512, 960))
        return result
